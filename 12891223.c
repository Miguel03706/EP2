...
/**   S.I (94) - Luciano Antonio Digiampietri                       **/
/**                                                                 **/
...
/*
TURMA: tem por objetivo representar uma turma. E uma estrutura que possui cinco campos:
codigoDisciplina do tipo inteiro que corresponde ao codigo numerico da disciplina,
nomeDisciplina do tipo ponteiro para caracteres que aponta para o nome da disciplina;
totalVagas do tipo inteiro que contem o numero de vagas oferecidas na turma;
numAlunos do tipo inteiro que contem o numero atual de alunos na turma; e
nusps do tipo ponteiro para inteiro que aponta para um arranjo que contem o numero USP de cada um dos alunos da turma.
*/
typedef struct
{
...
/*
DESEMPENHO: tem por objetivo representar o desempenho de um aluno em uma turma.  Possui quatro campos:
turma que é do tipo ponteiro para TURMA e aponta para a turma a que o desempenho se refere;
nota, do tipo inteiro, que contem a nota do aluno na respectiva turma (de 0 [zero] a 100);
frequencia do tipo inteiro que contem a frequencia do aluno na respectiva turma (de 0 [zero] a 100); e
status do tipo char cujo valor sera 'M' para o aluno recem matriculado na turma e podera ter outros dois valores possiveis
dependendo da nota e da frequencia do aluno
('A' para aprovado e 'R' para reprovado).
*/
typedef struct
{
    TURMA *turma;
...
/*
ALUNO: tem por objetivo representar um aluno e seu historico escolar. Possui cinco campos:
nome do tipo ponteiro para caracteres que aponta para o nome do aluno;
nusp do tipo inteiro que representa o numero USP do aluno;
historico do tipo arranjo de DESEMPENHO que corresponde ao historico escolar do aluno,
com suas matriculas e desempenho nas turmas;
turmasNoHistorico do tipo inteiro que indica em quantas turmas o aluno ja se matriculou; e
status do tipo char cujo valor sera 'M' para o aluno que ainda nao conclui seu curso e podera
ter outros dois valores possiveis dependendo de seu desempenho no conjunto de turmas
('F' para formado e 'J' para jubilado).
*/
typedef struct
{
    char *nome;
...
/*
CURSO: tem por objetivo representar um curso que e composto por turmas e alunos. Possui quatro campos:
alunos do tipo arranjo de ponteiros para ALUNOs;
numAlunos do tipo inteiro que armazena a quantidade de alunos no curso;
turma do tipo arranjo de ponteiros para TURMAs; e
numTurmas do tipo inteiro que armazena a quantidade de turmas no curso.
*/
typedef struct
{
...
/*
Funcao para adicionar um aluno (ou mais especificamente uma referencia a um aluno) no arranjo
de alunos do curso cuja referencia/endereco de memoria foi passado como parametro (curso).

A funcao devera retornar false caso o valor do endereco do parametro aluno ou do parametro
curso seja igual a NULL. Tambem devera retornar false caso o numero de alunos no curso ja seja
igual a MAX_ALUNOS_CURSO ou se o aluno referenciado por aluno ja pertenca ao arranjo de alunos
do curso (verifique isso com base no numero USP do aluno). Caso contrario, o endereco do aluno
passado como parametro deve ser inserido no arranjo de alunos, na posicao numAlunos e este campo
(apontado por curso) deve ser incrementado em 1 e a funcao deve retornar true.
*/
...
/*
Funcao para adicionar uma turma (ou mais especificamente uma referencia a uma turma) no arranjo
de turmas do curso cuja referencia/endereco de memoria foi passado como parametro (curso).
A funcao devera retornar false caso o valor do parametro turma ou do parametro curso seja igual a NULL.
Tambem devera retornar false caso o numero de turmas no curso ja seja igual a MAX_TURMAS_CURSO ou se a
turma referenciado por turma ja pertenca ao arranjo de turmas do curso (verifique isso com base no endereco
de memoria que consta na variavel turma, pois a mesma turma nao deve ser inserida duas vezes, mas e possivel
haver duas turmas diferentes de uma mesma disciplina). Caso contrario, a turma passada como parametro deve ser
inserida no arranjo de turmas, na posicao numTurmas e este campo (apontado por curso) deve ser incrementado em
...
se formar (DISCIPLINAS_NECESSARIAS) o campo status deve ser atualizado para 'J', pois nao sera mais
possivel se formar. Por fim, caso nenhuma dessas situacoes ocorra, o status deve permanecer 'M' (matriculado).
*/
// FIXME: FEITO
...
/*
Funcao para matricular um aluno em uma turma. A funcao devera retornar false caso o endereco aluno ou turma seja igual a NULL.
Caso contrario, a funcao devera chamar a funcao atualizarStatusAluno, passando aluno como parametro, isto sera necessario
para verificar o status atualizado do aluno.
    ...
    turma na posicao numAlunos e o campo numAlunos da turma deve ser incrementado em um; por fim, a funcao devera
    retornar true.
*/
// FIXME: FEITO
...
bool matricularAlunoEmTurma(ALUNO *aluno, TURMA *turma)
{
    ...
    // Verifica se já está matriculado em outra turma da msm disciplina
    for (int i = 0; i < aluno->turmasNoHistorico; i++)
    {
    ...
    // Verificar nº de alunos na turma
    if (turma->numAlunos == turma->totalVagas)
    {
        return false;
    }

    // Verifica se já atingiu o limite de turmas
    if (aluno->turmasNoHistorico == MAX_TURMAS)
    {
        return false;
    }

    // Add desempenho no histórico do aluno
...
int main()
{
    ...
    if (adicionarAlunoAoCurso(&a1, &meuCurso))
        printf("1. OK\n");
    else
        printf("NOK\n");
    if (adicionarAlunoAoCurso(&a2, &meuCurso))
        printf("2. OK\n");
    else
        printf("NOK\n");
    if (adicionarAlunoAoCurso(&a3, &meuCurso))
        printf("3. OK\n");
    else
        printf("NOK\n");
    if (adicionarAlunoAoCurso(&a4, &meuCurso))
        printf("4. OK\n");
    else
        printf("NOK\n");
    if (adicionarAlunoAoCurso(&a5, &meuCurso))
        printf("NOK\n");
    else
        printf("5. OK\n");
    if (adicionarAlunoAoCurso(&a6, &meuCurso))
        printf("6. OK\n");
    else
        printf("NOK\n");
    if (adicionarAlunoAoCurso(&a4, &meuCurso))
    ...
    if (adicionarTurmaAoCurso(&t1, &meuCurso))
        printf("8. OK\n");
    else
        printf("NOK\n");
    if (adicionarTurmaAoCurso(&t2, &meuCurso))
        printf("9. OK\n");
    else
        printf("NOK\n");
    if (adicionarTurmaAoCurso(&t3, &meuCurso))
        printf("10. OK\n");
    else
        printf("NOK\n");
    if (adicionarTurmaAoCurso(&t4, &meuCurso))
        printf("11. OK\n");
    else
        printf("NOK\n");
    if (adicionarTurmaAoCurso(&t5, &meuCurso))
        printf("12. OK\n");
    else
        printf("NOK\n");
    if (adicionarTurmaAoCurso(&t6, &meuCurso))
        printf("13. OK\n");
    else
        printf("NOK\n");
    if (adicionarTurmaAoCurso(&t3, &meuCurso))
    ...
    if (matricularAlunoEmTurma(&a1, &t1))
        printf("15. OK\n");
    else
        printf("NOK\n");
    if (matricularAlunoEmTurma(&a1, &t2))
        printf("16. OK\n");
    else
        printf("NOK\n");
    if (matricularAlunoEmTurma(&a1, &t3))
        printf("17. OK\n");
    else
        printf("NOK\n");
    if (matricularAlunoEmTurma(&a1, &t2))
        ...
        printf("18. OK\n");
    if (matricularAlunoEmTurma(&a2, &t1))
        printf("19. OK\n");
    else
        printf("NOK\n");
    if (matricularAlunoEmTurma(&a2, &t2))
        printf("20. OK\n");
    else
        printf("NOK\n");
    if (matricularAlunoEmTurma(&a3, &t1))
        printf("21. OK\n");
    else
        printf("NOK\n");
    if (matricularAlunoEmTurma(&a4, &t1))
        printf("NOK\n");
    else
        printf("22. OK\n");
    ...
    if (cadastrarNota(&a1, t1.codigoDisciplina, 100, 90))
        printf("23. OK\n");
    else
        printf("NOK\n");
    if (cadastrarNota(&a1, t2.codigoDisciplina, 70, 70))
        printf("24. OK\n");
    else
        printf("NOK\n");
    if (cadastrarNota(&a1, t3.codigoDisciplina, 40, 75))
        printf("25. OK\n");
    else
        printf("NOK\n");
    if (cadastrarNota(&a1, t2.codigoDisciplina, 80, 80))
        printf("NOK\n");
    else
        printf("26. OK\n");
    if (cadastrarNota(&a1, t4.codigoDisciplina, 90, 90))
        printf("NOK\n");
    else
        printf("27. OK\n");
    ...
    if (matricularAlunoEmTurma(&a1, &t3))
        ...
        printf("28. OK\n");
    if (matricularAlunoEmTurma(&a1, &t4))
        printf("29. OK\n");
    else
        printf("NOK\n");
    if (matricularAlunoEmTurma(&a1, &t5))
        printf("30. OK\n");
    else
        printf("NOK\n");
    if (matricularAlunoEmTurma(&a1, &t6))
        printf("31. OK\n");
    else
        printf("NOK\n");
    ...
    if (cadastrarNota(&a1, t4.codigoDisciplina, 100, 90))
        printf("32. OK\n");
    else
        printf("NOK\n");
    if (cadastrarNota(&a1, t5.codigoDisciplina, 70, 70))
        printf("33. OK\n");
    else
        printf("NOK\n");
    if (cadastrarNota(&a1, t6.codigoDisciplina, 60, 100))
        printf("34. OK\n");
    else
        printf("NOK\n");
    if (cadastrarNota(&a1, t3.codigoDisciplina, 80, 80))
        printf("NOK\n");
    else
        printf("35. OK\n");
    if (cadastrarNota(&a1, t4.codigoDisciplina, 90, 90))
