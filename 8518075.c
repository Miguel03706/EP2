...
{
	int i;
	if (aluno == NULL || curso == NULL) // testa se o end dos parametros sao NULL
		return 0;
	if (curso->numAlunos == MAX_ALUNOS_CURSO) // testa se o numero de alunos e igual ao maximo de alunos amtriculados no curso
		return 0;
	for (i = 0; i < curso->numAlunos; i++)
		if (curso->alunos[i]->nusp == aluno->nusp) // testa se existe um aluno matriculado no curso com o mesmo nusp
			return 0;
...
bool adicionarTurmaAoCurso(TURMA *turma, CURSO *curso)
{
	...
	if (curso->numTurmas == MAX_TURMAS_CURSO)
		return 0;
	for (i = 1; i < curso->numTurmas; i++)
		if (turma == curso->turmas[i])
			return 0;
...
bool cadastrarNota(ALUNO *aluno, int codigoDisciplina, int nota, int freq)
{
	...
	for (i = 0; i < aluno->turmasNoHistorico; i++)
	// itera entre os desempenhos nas turmas no historico
	{

		if (aluno->historico[i].turma->codigoDisciplina == codigoDisciplina && aluno->historico[i].status == 'M')
		{										   // testa se o aluno referenciado pelo parametro aluno possui um DESEMPENHO em seu historico escolar(historico) para a disciplina com codigo igual ao valor do parametro codigoDisciplina e cujo status seja iguala‘M’
			aluno->historico[i].frequencia = freq; // atribui a frequencia
			aluno->historico[i].nota = nota; // atribui a nota
			if (nota >= 50 && freq >= 70)
				aluno->historico[i].status = 'A'; // testa se o aluno tem nota e frequencia para ser aprovado
			else
				aluno->historico[i].status = 'R'; // atribui para reprovado caso não tenha nota e frequencia
			return 1;
		}
...
void atualizarStatusAluno(ALUNO *aluno)
{
	...
	int dAprovado = 0;
	for (i = 0; i < aluno->turmasNoHistorico; i++)
	{ // soma a quantidade de aprovações
	...
	int dReprovado = 0;
	for (i = 0; i < aluno->turmasNoHistorico; i++)
	{ // soma a quantidade de reprovações
		if (aluno->historico[i].status == 'A')
			dReprovado++;
	}

	if (dAprovado >= DISCIPLINAS_NECESSARIAS)
	{
		aluno->status = 'F';
	}
	else if ((MAX_TURMAS - dReprovado) < DISCIPLINAS_NECESSARIAS)
	{
		aluno->status = 'J';
	}
	else
...
bool matricularAlunoEmTurma(ALUNO *aluno, TURMA *turma)
{
	...
	if (aluno == NULL || turma == NULL)
		return 0; // testa de os parametros recebidos são NULL
	else
		atualizarStatusAluno(aluno); // Atualiza o status do aluno para testes

	if (turma->numAlunos == turma->totalVagas || aluno->status != 'M' || aluno->turmasNoHistorico == MAX_TURMAS)
		return 0; // testa se a turma já está lotada, se o aluno possui status geral diferente de matriculado e se ele possui alcançou o maximo de turmas possiveis para cursar
	for (i = 0; i < turma->numAlunos; i++)
		if (turma->nusps[i] == aluno->nusp)
			return 0; // itera i para verificar se o nusp do aluno já consta na turma (ja esta matriculado)
	for (i = 0; i < aluno->turmasNoHistorico; i++)
		if (turma->codigoDisciplina == aluno->historico[i].turma->codigoDisciplina)
			for (i = 0; i < aluno->turmasNoHistorico; i++)
				if (aluno->historico[i].status != 'R')
					return 0;

	aluno->historico[aluno->turmasNoHistorico] = novoDesempenho(turma); // adiciona novo desempenho no historico do aluno
	aluno->turmasNoHistorico++;											// incrementa o numero de turmas cursadas pelo aluno
	turma->nusps[turma->numAlunos] = aluno->nusp;						// atribui
	turma->numAlunos++;													// incrementa o numero de alunos na turma
	return 1;
}
